    ...
    <div className="page-container">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {list.map((h) => (
            <div key={h.id} className="list-item bg-white dark:bg-slate-800">
              <button
                type="button"
              ...
              <div className="flex-1 min-w-0">
                ...
                {h.process && (
                  <Link to={`/processos/${h.process.id}`} className="text-xs text-primary-600 hover:underline font-mono mt-1 inline-block">
                    {h.process.number}
                  </Link>
                )}
              </div>
              {h.type && (
                <span className="text-xs px-2 py-1 rounded bg-slate-200 dark:bg-slate-600 text-slate-600 dark:text-slate-300 self-start sm:self-center shrink-0">
                  {h.type}
                </span>
              )}
            </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Nova audiência">
